    ...
    (case-lambda
      ...
      ((binop x y)
       (case binop
         ((+) `(+ ,x ,y))
         ((-) `(- ,x ,y))
         ((*) (cond ((number? x) `(* ,(sra x 3) ,y))
                    ((number? y) `(* ,x ,(sra y 3)))
                    (else `(* ,x (sra ,y 3)))))
         ((cons)
          (define cart (unique-symbol 'car))
          (define cdrt (unique-symbol 'cdr))
          (define pairt (unique-symbol 'pair))
          `(let ((,cart ,x) (,cdrt ,y))
             (let ((,pairt (+ (alloc ,size:pair) ,tag:pair)))
               (begin
                 (mset! ,pairt ,(- offset:car tag:pair) ,cart)
                 (mset! ,pairt ,(- offset:cdr tag:pair) ,cdrt)
                 ,pairt))))
         ((vector-ref)
          (if (number? y)
